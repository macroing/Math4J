...
public final class SurfaceSample3F {
	...
	public String toString() {
		return String.format("new SurfaceSample3F(%s, %s, %s)", this.point, this.surfaceNormal, Float.toString(this.probabilityDensityFunctionValue));
	}
	...
	public SurfaceSample3F transformToObjectSpace(final Matrix44F objectToWorld, final Matrix44F worldToObject) {
		final Point3F point = this.point.transform(worldToObject);
		
		final Vector3F surfaceNormal = this.surfaceNormal.transformTranspose(objectToWorld);
		
		final float probabilityDensityFunctionValue = this.probabilityDensityFunctionValue;
		
		return new SurfaceSample3F(point, surfaceNormal, probabilityDensityFunctionValue);
	}
	...
	public SurfaceSample3F transformToWorldSpace(final Matrix44F objectToWorld, final Matrix44F worldToObject) {
		final Point3F point = this.point.transform(objectToWorld);
		
		final Vector3F surfaceNormal = this.surfaceNormal.transformTranspose(worldToObject);
	...
	public boolean equals(final Object object) {
		if(object == this) {
			...
			return false;
		} else if(!MathF.equals(this.probabilityDensityFunctionValue, SurfaceSample3F.class.cast(object).probabilityDensityFunctionValue)) {
			return false;
		} else {
			return true;
		}
